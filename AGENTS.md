...
•    본 프로젝트는 iOS 전용이며, 테스트 실행은 오직 로컬 macOS CLI 환경에서만 수행된다. Codex는 테스트 코드 작성과 구조 설계까지만 담당하며, swift test/build/run 명령은 절대 실행하지 않는다. 테스트 실패를 피하기 위한 조건부 전처리, 임시 타입 정의, 빈 구현 함수 등은 절대 작성하지 않으며, 모든 판단 기준은 책임 단위 분리에 기반해야 한다.
