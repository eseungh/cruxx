...
(프로젝트 구조, 기능 범위, AI 엔진, 협업 원칙, Xcode 기반 필수)
...
•    빌드/테스트/실행은 기본적으로 Xcode 빌드/실행/테스트 단축키 (⌘B, ⌘R, ⌘U)
...
•    브랜치 명명은 영어
•    새로운 기능은 반드시 기존 구조(MVVM, DI, 모듈화, Xcode) 내에서 구현
•    주요 로직은 Core/Service, 재사용 UI는 Shared/Components에 위치
•    작업 내역, 이슈, 설명, 코드 리뷰 모두 Notion 및 저장소에 기록
•    오픈채팅·Codex·AI 에이전트 협업 시에도 동일 기준 적용
•    본 프로젝트는 iOS 전용이며, 테스트 실행은 오직 로컬 macOS CLI 환경에서만 수행된다. Codex는 테스트 코드 작성과 구조 설계까지만 담당하며, Xcode에서의 실행/테스트만 수행하며, CLI 테스트는 사용하지 않는다. 테스트 실패를 피하기 위한 조건부 전처리, 임시 타입 정의, 빈 구현 함수 등은 절대 작성하지 않으며, 모든 판단 기준은 책임 단위 분리에 기반해야 한다.
...
- 모든 개발/수정/생성/삭제 작업은  
  **반드시 미리 지정된 폴더(예: Features/Recording 등) 내에서만 수행해야 하며,  
...
> Unless otherwise specified, all code changes must be strictly limited to the directory explicitly specified in each task request (e.g., Features/Recording).  
