...
```
  ...
  └── cruxxCore   # ViewModel 및 Core 로직 테스트
```

* 아키텍처: MVVM + DI + Modular (Xcode 기반)
* 주요 폴더 구조 기반: App, Core, Model
...
> ❌ Codex는 `⌘U (Xcode Test)` 또는 `⌘B (Xcode Build)`를 직접 실행하지 않습니다.
...
* Project Type: Xcode (.xcodeproj only)
* 동작 분석: AVFoundation + Custom Pose AI
* 테스트: XCTest + Xcode 기반 유닛 테스트
